<script>
	let { removeLetter, taskTranslation, searchValue} = $props()
    function count () {
		if (taskTranslation) return taskTranslation
        return searchValue
    }

</script>
<button class=" ml-2 backspace flex items-center justify-center" onclick={()=>removeLetter(count())}>
